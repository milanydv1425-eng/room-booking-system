...
function MyBookings() {
  ...
  const fetchBookings = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/bookings/my/${user.id}`);
  ...
  const handleCancel = async (bookingId) => {
    ...
    try {
      ...
      setCancelError((prev) => ({
        ...prev,
        [bookingId]: error.response?.data?.message || "Failed to cancel booking.",
      }));
    }
  };
    ...
    <div className="min-h-screen bg-stone-950 relative overflow-hidden">
      ...
      <div className="relative z-10 border-b border-stone-800 bg-stone-950/80 backdrop-blur-sm px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-stone-100 font-black tracking-tight" style={{ fontFamily: "'Georgia', serif" }}>
            Room Booking
          </span>
        </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-stone-500 text-sm hidden sm:block">
            Hey, <span className="text-amber-400 font-semibold">{user?.name}</span>
          </span>
          <Link to="/dashboard">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-12">
        <div className="mb-10">
          <h1 className="text-4xl font-black text-stone-100 mb-2" style={{ fontFamily: "'Georgia', serif", letterSpacing: "-0.02em" }}>
            My <span className="bg-gradient-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent">Bookings</span>
          </h1>
          <p className="text-stone-500 text-sm tracking-widest uppercase">Your reserved rooms</p>
        </div>

        {bookings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            <div className="w-16 h-16 rounded-2xl bg-stone-900 border border-stone-800 flex items-center justify-center">
              <svg className="w-8 h-8 text-stone-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
              </svg>
            </div>
            <p className="text-stone-600 text-sm tracking-widest uppercase">No bookings yet</p>
            <Link to="/dashboard">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {bookings.map((booking) => (
              <div key={booking.id} className="bg-stone-900/60 border border-stone-800 rounded-2xl p-6 backdrop-blur-sm flex flex-col gap-4 hover:border-stone-700 transition-all duration-200">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-stone-100 font-bold text-lg" style={{ fontFamily: "'Georgia', serif" }}>{booking.name}</h3>
                    <p className="text-amber-400 font-black text-xl mt-1">
                      ₹{booking.price_per_night}
                      <span className="text-stone-500 text-xs font-normal ml-1">/ night</span>
                    </p>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-stone-500 text-xs tracking-widest uppercase">Check-in</span>
                    <span className="text-stone-300 text-sm font-medium">
                      {new Date(booking.start_date).toLocaleDateString("en-IN", { year: "numeric", month: "short", day: "numeric" })}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-stone-500 text-xs tracking-widest uppercase">Check-out</span>
                    <span className="text-stone-300 text-sm font-medium">
                      {new Date(booking.end_date).toLocaleDateString("en-IN", { year: "numeric", month: "short", day: "numeric" })}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-3 py-2">
                  ...
                  <span className="text-emerald-400 text-xs tracking-widest uppercase font-semibold">Confirmed</span>
                </div>
                ...
                {cancelError[booking.id] && (
                  <div className="flex items-start gap-2 bg-red-500/10 border border-red-500/30 rounded-xl px-3 py-2">
                    ...
                    <p className="text-red-400 text-xs">{cancelError[booking.id]}</p>
                  </div>
                )}
                {cancelSuccess[booking.id] && (
                  <div className="flex items-start gap-2 bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-3 py-2">
                    ...
                    <p className="text-emerald-400 text-xs">{cancelSuccess[booking.id]}</p>
                  </div>
                )}
